...
def understand_data(df, plot_dir="./notebooks/buddhika/eda_plots/"):
    """
...
def preprocess_data(df):
    ...
    if df['depth'].isnull().sum() > 0:
        missing_depth_count = df['depth'].isnull().sum()
        median_depth = df['depth'].median()
        df['depth'] = df['depth'].fillna(median_depth)
        print(f"Filled {missing_depth_count} missing values in 'depth' with median ({median_depth}).")
        
    # 4. Handle outliers in carat and price using IQR method
    ...
    # 5. Encode cut, color, clarity as ordinal ints based on config.py
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(project_root, "data", "cubic_zirconia.csv")
    ...
    if os.path.exists(data_path):
        ...
        plot_dir = os.path.join(project_root, "notebooks", "buddhika", "eda_plots")
        understand_data(df, plot_dir=plot_dir)
        
        df_clean = preprocess_data(df)
        
        clean_path = os.path.join(project_root, "data", "gemstone_clean.csv")
